...
{
    if (size == 0 || (size > BLB_BLOCK_MAX_SIZE))
        return NULL;
...
bool blb_blob_create_new_range(blb_blob_t *blob, uint32_t start, uint32_t size, uint8_t step, bool fixed, uint8_t index)
{
    if (!blob || !blob->range)
        return false;
    blob->range[index] = *(blb_range_create(start, size, step, fixed));
    return true;
}
...
bool blb_blob_range_slide(blb_blob_t *blob, int32_t steps, uint8_t index)
{
    if (!blob || !blob->range)
        return false;
    return blb_range_slide((&(blob->range[index])), steps);
}
...
bool blb_blob_range_resize(blb_blob_t *blob, int32_t size, uint8_t index)
{
    if (!blob || !blob->range)
        return false;
    return blb_range_resize((&(blob->range[index])), size);
}
